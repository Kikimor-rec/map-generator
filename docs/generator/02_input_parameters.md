...
- `archetype`:  
  `ship | station | outpost | asteroidMine | beaconRelay | derelict | factoryModule | prisonTransport | habitatCylinder | megaringSegment | colonyBlock | listeningPost`

- `subtype`: зависит от `archetype`, см. 13_archetypes_subtypes.md

- `styleProfile`: `realism | futurism`

- `sizeTier`: `xs | s | m | l | xl`  
...
## 2.4 Уровни/палубы
- `decks`: number | "auto"  
- `maxRoomsPerDeck`: number (опционально)  
- `verticality`: number 0..1 (вероятность вертикальных обходов/шахт)

## 2.5 Контроль содержания
- `roomCountTarget`: number (опционально) — если задано, приоритет над sizeTier.
- `includeList`: string[] (типы комнат) — принудительно добавить.
- `excludeList`: string[] — запретить (например, teleportRoom в realism).
- `mustHaveSetpieces`: string[] (типы) — например `["hangar","reactorHall"]`
- `accessPolicy`: "soft"|"strict"  
  strict: secure зоны обязательно отделены checkpoint/bulkhead.

## 2.6 Ограничения (hard)
